...
static int bar(const char *re, int re_len, const char *s, int s_len,
	...
	for (i = j = 0; i < re_len && j <= s_len; i += step) {
		...
		step = re[i] == '(' ?
				info->brackets[bi + 1].len + 2 : get_op_len(re + i, re_len - i);
		...
		if (i + step < re_len && is_quantifier(re + i + step)) {
			...
			if (re[i + step] == '?') {
				...
				if (n1 < 0 && n2 < 0 && re[i + step] == '*'
						&& (n2 = bar(re + ni, re_len - ni, s + j, s_len - j,
								info, bi)) > 0) {
					nj = j + n2;
				}
		...
		if (re[i] == '[') {
			...
			n = SLRE_NO_MATCH;
			bi++;
			FAIL_IF(bi >= info->num_brackets, SLRE_INTERNAL_ERROR); DBG(("CAPTURING [%.*s] [%.*s] [%s]\n",
	...
	for (i = 0; i < re_len; i += step) {
		...
		if (re[i] == '|') {
			...
			if (re[i + 1] == 'x') {
				...
				FAIL_IF(
						re[i + 1] == 'x' && !(isxdigit(re[i + 2]) && isxdigit(re[i + 3])),
						SLRE_INVALID_METACHARACTER);
			} else {
...
int slre_match(const char *regexp, const char *s, int s_len,
		struct slre_cap_t *caps, int num_caps, int flags) {
	struct regex_info info;

//  printf("slre_match(reg=%s,str=%s,len=%i) = ",regexp,s,s_len);
	...
	DBG(("========================> [%s] [%.*s]\n", regexp, s_len, s));
	int res = foo(regexp, (int) strlen(regexp), s, s_len, &info);
//  printf("%i\n\r",res);
	return res;
}
