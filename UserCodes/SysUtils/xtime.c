...
/* Приводится пример реализации на языке Си функций конвертации между UNIX-временем
 * и обычным представлением в виде даты и времени суток.
 * Пример приведен в стандартном знаковом 32-бит формате. Однако,
 * закомментировав определение _XT_SIGNED [xtime.h:8], пример соберется в беззнаковом варианте. */
#include "xtime.h"
...
void xttotm(datetime_t *t, xtime_t secsarg)
{
u32_t		secs;
s32_t		days;
s32_t		mon;
...
const s16_t*	pm;
	...
	unsigned long jd = ((secsarg+43200)/(86400>>1)) + (2440587<<1) + 1;
	unsigned long jdn = jd>>1;
